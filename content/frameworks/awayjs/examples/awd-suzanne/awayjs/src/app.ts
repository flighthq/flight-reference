import { LoaderEvent, Vector3D, AssetLibrary, Loader, URLRequest, RequestAnimationFrame } from '@awayjs/core';
...
class AWDSuzanne {
  ...
  private render(_dt: number) {
    if (this._scene.camera) {
      ...
      this._light.direction = new Vector3D(
        -Math.cos(-this._cameraIncrement) * 1400,
        0,
        -Math.sin(this._cameraIncrement) * 1400,
      );
    }
    ...
    for (let i: number = 0; i < numAssets; ++i) {
      ...
      switch (asset.assetType) {
        case Sprite.assetType: {
          ...
          this._suzane = sprite;
          (this._suzane.material as MethodMaterial).lightPicker = this._lightPicker;
          this._suzane.y = -100;
          this._mouseOutMaterial = this._suzane.material as MethodMaterial;

          for (let c: number = 0; c < 80; c++) {
            const clone = sprite.clone() as Sprite;
            const scale: number = this.getRandom(50, 200);
  ...
  private onMouseOver(event: MouseEvent) {
    (event.entity as Sprite).material = this._mouseOverMaterial;
    console.log('mouseover');
  }

  private onMouseOut(event: MouseEvent) {
    (event.entity as Sprite).material = this._mouseOutMaterial;
