...
manifest.addFont('openfl/fonts/KatamotzIkasi.ttf', declaredFontName);

AssetLibrary.loadFromManifest(manifest)
  .onComplete((library) => {
    Assets.registerLibrary('default', library);

    const font = Assets.getFont(declaredFontName) ?? Assets.getFont('openfl/fonts/KatamotzIkasi.ttf');
    if (font?.fontName) loadedFontName = font.fontName;

    Assets.loadFont(declaredFontName)
      .onComplete(async () => {
        if ('FontFace' in window && 'fonts' in document) {
          const fontFace = new FontFace(declaredFontName, 'url(openfl/fonts/KatamotzIkasi.ttf)');
          await fontFace.load();
