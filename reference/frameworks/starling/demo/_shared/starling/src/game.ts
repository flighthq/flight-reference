...
class Game extends Sprite {
  ...
  private onKey = (event: KeyboardEvent): void => {
    if (event.keyCode == Keyboard.SPACE) Starling.current.showStats = !Starling.current.showStats;
    else if (event.keyCode == Keyboard.X) Starling.current.context.dispose();
  };

  private onButtonTriggered = (event: Event): void => {
    var button: Button = event.target as Button;

    if (button.name == 'backButton') this.closeScene();
    else this.showScene(button.name);
  };
  ...
  public startScene(assets: AssetManager, SceneClass: new () => Scene): void {
    Game.sAssets = assets;
    this.addChild(new Image(assets.getTexture('background')));
    this._currentScene = new SceneClass();
    this.addChild(this._currentScene);
