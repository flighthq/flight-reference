...
class BenchmarkScene extends Scene {
  ...
  private onEnterFrame = (event: EnterFrameEvent, passedTime: number): void => {
    ...
    for (var i = 0; i < BenchmarkScene.FRAME_TIME_WINDOW_SIZE; i++) this._frameTimes[i] += passedTime;
    ...
    if (this._phase == 0) {
      if (measuredFps < 0.985 * this._targetFps) {
        ...
        if (this._failCount == BenchmarkScene.MAX_FAIL_COUNT) this._phase = 1;
      } else {
    ...
    if (this._phase == 1) {
      if (measuredFps > 0.99 * this._targetFps) {
        ...
        if (this._failCount == 0) this.benchmarkComplete();
      } else {
  ...
  private removeTestObjects(count: number): void {
    ...
    if (count >= numChildren) count = numChildren;
  ...
  private getObjectFromPool(): DisplayObject {
    ...
    if (this._objectPool.length == 0) {
      ...
      return image;
    } else return this._objectPool.pop();
  }
