...
manifest.addFont('openfl/fonts/LiberationSerif-Regular.ttf', 'Liberation Serif Regular');
manifest.addFont('openfl/fonts/nokiafc22.ttf', 'Nokia Cellphone FC Small');

for (var renderer of ['flash', 'legacy', 'html5']) {
  for (var i = 0; i <= 5; i++) {
    manifest.addBitmapData('openfl/img/' + renderer + i + '.png');
  }
}
    ...
    Promise.all([loadFont('Liberation Serif Regular'), loadFont('Nokia Cellphone FC Small')])
      .then(async () => {
        if ('FontFace' in window && 'fonts' in document) {
          const liberation = new FontFace('Liberation Serif Regular', 'url(openfl/fonts/LiberationSerif-Regular.ttf)');
          const nokia = new FontFace('Nokia Cellphone FC Small', 'url(openfl/fonts/nokiafc22.ttf)');
